class BipedalJumper {
  constructor(params = {}) {
    ...
    this.score = 0;
    this.fitness = 0;
    this.brain = new NeuralNetwork(8, 25, 2);
  ...
  /**
   * Returns an object of with all the parameters required to create a new BipedalJumper
   * @returns {Object}
   */
  getParams() {
  ...
  adjustScore() {
    const isHeadBalanced = Math.abs(this.body.angle) < 0.30;
    const leftLegTouchesGround = (boundary.ground.bounds.min.y - this.leftLeg.bounds.max.y) < 1;
    const rightLegTouchesGround = (boundary.ground.bounds.min.y - this.rightLeg.bounds.max.y < 1);
    const bodyTouchesGround = (boundary.ground.bounds.min.y - this.body.bounds.max.y) < 1;
    const jumpingScore = this.body.position.y
      * (leftLegTouchesGround ? 2 : 0.1)
      * (rightLegTouchesGround ? 2 : 0.1)
      * (bodyTouchesGround ? 2 : 0.1)
      * (isHeadBalanced ? 1 : 0.1);
  ...
  think(boundary) {
    ...
    // Move Muscles
    let leftMuscleShift = result[0] > 0.5 ? 10 : -10;
    let rightMuscleShift = result[1] > 0.5 ? 10 : -10;

    if (this.leftMuscle.length + leftMuscleShift < 70 && this.leftMuscle.length + leftMuscleShift > 15)
      this.leftMuscle.length += leftMuscleShift;
    if (this.rightMuscle.length + rightMuscleShift < 70 && this.rightMuscle.length + rightMuscleShift > 15)
